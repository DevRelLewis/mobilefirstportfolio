...
function isPortfolioRelatedQuery(query) {
  ...
  const isAboutQuery = aboutQueries.some((phrase) =>
...
function generateNaturalFallback(query, filteredDocs, citationList) {
  ...
  if (isIntroductionQuery(query)) {
    return `This is Lewis Meyers' portfolio! He's a Full Stack Engineer and Army Veteran with a strong background in building AI-driven applications. Lewis has worked at companies like TopShelfIntelligence, Teachstone, and Hyperlink, where he's delivered impactful projects using React, Next.js, TypeScript, and Python [${citationList
      .slice(0, 2)
      .join(", ")}]. Want to know more about his experience or projects?`;
  }

  if (lowerQuery.includes("experience") || lowerQuery.includes("work")) {
    return `Lewis has great experience across several companies! Most recently, he's been a Web Developer & SEO Strategist at TopShelfIntelligence and a Software Engineer at Teachstone, where he improved platform performance by 30% and boosted user adoption by 40% [${citationList
      .filter((c) => c.includes("exp"))
      .slice(0, 2)
      .join(
        ", "
      )}]. He's also worked on AI platforms at Hyperlink, helping raise $3.2M at SXSW 2024!`;
  }

  if (lowerQuery.includes("project") || lowerQuery.includes("built")) {
    return `Lewis has built some cool projects! There's Dream Net AI, an AI dream interpreter using Next.js and ChatGPT with 25% improved accuracy, and the Soke2x Smoothie Shop featuring 3D elements with Three.js [${citationList
      .filter((c) => c.includes("project"))
      .slice(0, 2)
      .join(
        ", "
      )}]. He's also working on a Spring Boot Pokedex with Java and Kafka!`;
  }
  ...
  if (
    ...
    return `Lewis has a solid tech stack! He's proficient in JavaScript, TypeScript, React, Next.js, Python, and has experience with AI/ML including prompt engineering and RAG workflows [${citationList
      .filter((c) => c.includes("skill"))
      .join(
        ", "
      )}]. He's comfortable with both frontend and backend development, plus cloud tools like AWS and databases like PostgreSQL.`;
  }
  ...
  if (
    ...
    return `You can reach Lewis at meyerslewis193@gmail.com or connect with him on LinkedIn [${citationList
      .filter((c) => c.includes("contact"))
      .join(
        ", "
      )}]. His portfolio website is lewismeyers.dev if you'd like to see more of his work!`;
  }

  return `Hey! I'm here to tell you about Lewis Meyers' professional background. ${filteredDocs[0]?.content.substring(
    0,
    120
  )}... [${citationList[0]}]. What would you like to know more about?`;
}

// RAG Proxy Handler
async function handleRAGQuery(req, res, requestId) {
  const { message, config = {} } = req.body;

  try {
    const response = await fetch(`${RAG_API_BASE}/api/query`, {
      method: "POST",
      headers: {
    ...
    console.log(
      `[${requestId}] RAG response received with ${
    ...
    return {
      ...
      citations:
        ragData.citations?.map(
          (cite) => cite.source_system + ":" + cite.file_path.split("/").pop()
        ) || [],
      confidence: ragData.citations?.length > 0 ? 0.9 : 0.1,
      source_breakdown: ragData.citations?.reduce((acc, cite) => {
        acc[cite.source_system] = (acc[cite.source_system] || 0) + 1;
        return acc;
      }, {}),
      _trace: {
        used_rag: true,
        rag_citations_count: ragData.citations?.length || 0,
        timestamp: new Date().toISOString(),
      },
    };
  } catch (error) {
    console.error(`[${requestId}] RAG API error:`, error.message);
...
export default async function handler(req, res) {
  ...
  // Handle RAG upload proxy
  if (req.url?.includes("/api/rag/upload")) {
    console.log(`[${requestId}] Proxying upload to RAG system`);
    try {
      const response = await fetch(`${RAG_API_BASE}/api/upload`, {
        method: "POST",
        headers: req.headers,
        body: req.body,
      });
      ...
      console.error(`[${requestId}] Upload proxy error:`, error);
      return res.status(500).json({ error: "Upload proxy failed" });
    }
  }

  // Handle RAG query proxy OR use enhanced system
  if (req.url?.includes("/api/rag/query") || req.body?.use_rag === true) {
    console.log(`[${requestId}] Using RAG system for query`);
    try {
      const ragResult = await handleRAGQuery(req, res, requestId);
      if (ragResult) {
        res.setHeader("Access-Control-Allow-Origin", "*");
      ...
      console.error(
        `[${requestId}] RAG system failed, falling back to original`
      );
    }
  }
  ...
  try {
    ...
    if (!isPortfolioRelatedQuery(message)) {
      ...
      return res.status(200).json({
        answer:
          "I'm Lewis's portfolio assistant and can only answer questions about his professional experience, skills, projects, and background. Please ask me something related to Lewis's work or qualifications!",
        citations: [],
        confidence: 1.0,
        _trace: {
    ...
    // Try RAG system first for portfolio queries
    const ragResult = await handleRAGQuery(req, res, requestId);
    if (ragResult) {
      res.setHeader("Access-Control-Allow-Origin", "*");
      return res.status(200).json(ragResult);
    ...
    console.log(
      `[${requestId}] RAG system unavailable, using original knowledge base`
    );
    ...
    console.log(
      `[${requestId}] Retrieved ${retrievedDocs.length} relevant docs`
    );
    console.log(
      `[${requestId}] Top doc scores:`,
      retrievedDocs.slice(0, 3).map((d) => ({ id: d.doc_id, score: d.score }))
    );
    ...
    if (config.filters) {
      ...
      filteredDocs = retrievedDocs.filter((doc) => {
        if (config.filters.type && doc.metadata.type !== config.filters.type)
          return false;
        if (
          config.filters.company &&
          doc.metadata.company !== config.filters.company
        )
          return false;
        if (
          config.filters.category &&
          doc.metadata.category !== config.filters.category
        )
          return false;
        return true;
      });
      console.log(
        `[${requestId}] After filtering: ${filteredDocs.length} docs`
      );
    }

    if (
      filteredDocs.length === 0 ||
      filteredDocs.every((doc) => doc.score === 0)
    ) {
      console.log(
        `[${requestId}] No relevant docs found, applying fallback strategy`
      );
      ...
      if (
        ...
        filteredDocs = kb
          ...
          .slice(0, 2);
        filteredDocs = filteredDocs.concat(
          kb.filter((doc) => doc.metadata.type === "contact")
        );
      } else if (
        query.includes("can") ||
        query.includes("able") ||
        ...
        filteredDocs = kb.filter(
          (doc) =>
            doc.metadata.type === "skills" || doc.metadata.type === "experience"
        );
      } else if (
        query.includes("work") ||
        query.includes("job") ||
        ...
        const experienceDocs = kb
          .filter((doc) => doc.metadata.type === "experience")
          .slice(0, 1);
        const projectDocs = kb
          .filter((doc) => doc.metadata.type === "project")
          .slice(0, 1);
        const skillsDocs = kb
          .filter((doc) => doc.metadata.type === "skills")
          .slice(0, 1);
      ...
      console.log(
        `[${requestId}] Fallback strategy provided ${filteredDocs.length} docs`
      );
    }

    const context = filteredDocs
      .map((doc) => `[${doc.doc_id}] ${doc.content}`)
      .join("\n\n");
    ...
    if (filteredDocs.length === 0) {
      ...
      return res.status(200).json({
        answer:
          "I don't have enough information to answer that question. You can ask me about Lewis's experience, projects, or skills.",
        citations: [],
        confidence: 0.0,
        _trace: {
          retrieved_docs: [],
...
Do not include any text outside the JSON object.`,
      ...
      console.log(
        `[${requestId}] Tokens used: prompt=${completion.usage?.prompt_tokens}, completion=${completion.usage?.completion_tokens}, total=${completion.usage?.total_tokens}`
      );

      const llmContent = completion.choices[0].message.content.trim();
      console.log(
        `[${requestId}] LLM response length: ${llmContent.length} characters`
      );
      ...
      try {
        ...
        console.log(
          `[${requestId}] Response confidence: ${response.confidence}`
        );
        console.log(
          `[${requestId}] Citations: ${response.citations?.length || 0}`
        );
        ...
        console.error(
          `[${requestId}] Failed to parse LLM response:`,
          parseError.message
        );
        console.error(
          `[${requestId}] Raw LLM content:`,
      ...
      const mockAnswer = generateNaturalFallback(
        message,
        filteredDocs,
        citationList
      );
      ...
      console.log(
        `[${requestId}] Generated fallback response: ${mockAnswer.substring(
          0,
          100
        )}...`
      );
    ...
    console.log(
      `[${requestId}] Total processing time: ${Date.now() - startTime}ms`
    );
    console.log(
